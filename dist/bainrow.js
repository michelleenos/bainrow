...
let o = null, l = null;
const u = () => o || (o = Object.keys(i).map((a) => i[a]), o), m = () => {
  if (l) return l;
  let t = /* @__PURE__ */ new Set(), a = [], g = Object.keys(k);
  const c = (e, s) => t.has(`${e}-${s}`.toLowerCase()) || t.has(`${s}-${e}`.toLowerCase()) ? !0 : (t.add(`${e}-${s}`.toLowerCase()), !1);
  return g.forEach((e) => {
    let s = k[e];
    if (s.pairs) {
      let { lights: n, darks: d, add: b } = s.pairs;
      n.forEach((f) => {
        d.forEach((r) => {
          c(f, r) || a.push([f, r]);
        });
...
function y(t = !0, a) {
  let g = u(), c = [];
  return g.forEach((e) => {
    let s = e.name, n = e.contexts, d = /* @__PURE__ */ new Set(), b = 0;
    n.forEach((f) => {
      let r = f.bg;
      if (d.has(r)) return;
      d.add(r);
      let h = t ? e.colors.filter((p) => p !== r) : e.colors;
      a && h.length < a || c.push({
        name: `${s}-${b++}`,
        colors: h,
        bg: r
...
export {
  m as getColorPairs,
  u as getPalettesArray,
  y as getPalettesWithBg,
  k as palettes
};
